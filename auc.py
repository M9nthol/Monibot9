...
def get_auction_markup():
    ...
    button_obv = types.InlineKeyboardButton(text="Обвесы", callback_data="obv")
    button_zap = types.InlineKeyboardButton(text="Заточка", callback_data="zap")
    markup.add(button_armor, button_weapon, button_cont, button_ddt, button_art, button_obv, button_zap)
    return markup

@bot.callback_query_handler(func=lambda call: call.data == "zap")
def handle_callback_query_prem(call):
    chat_id = call.message.chat.id
    # Create an inline keyboard with weapon options
    markup = types.InlineKeyboardMarkup(row_width=1)
    button_a92 = types.InlineKeyboardButton(text="Стандартные инструменты", callback_data="a92")
    button_a93 = types.InlineKeyboardButton(text="Стандартные запачасти", callback_data="a93")
    button_a94 = types.InlineKeyboardButton(text="Продвинутые инструменты", callback_data="a94")
    button_a95 = types.InlineKeyboardButton(text="Продвинутые запчасти", callback_data="a95")
    button_a96 = types.InlineKeyboardButton(text="Катализаторы", callback_data="a96")

    markup.add(button_a92, button_a93, button_a94, button_a95, button_a96)
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Хорошо! Выбери нужный предмет:",
                          reply_markup=markup)
...
def handle_callback_query_master_armor(call):
    ...
    button_a89 = types.InlineKeyboardButton(text="Альбатрос-Перехватчик", callback_data="a89")
    button_back = types.InlineKeyboardButton(text="Назад⬇️", callback_data="back")
    markup.add(button_a11, button_a12, button_a13, button_a14, button_a15, button_a89, button_back)
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Хорошо! Выбери броню:",
                          reply_markup=markup)
